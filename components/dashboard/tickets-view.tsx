...
// Defined OUTSIDE TicketsView to prevent remount on re-render
function CreateTicketForm({ onDone, onCancel }: { 
  ...
  const submit = async (e?: any) => {
    ...
    try {
      setSaving(true)
      setErr(null)
      const res = await maintenanceApi.createTicket(description, category, room)
      if (res.data?.success) {
        onDone(true)
      } else {
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full p-3 rounded-lg border">
      ...
      <div className="flex justify-end gap-3">
        ...
        <button type="submit" disabled={saving} className="px-4 py-2 bg-[#4D5D53] text-white rounded-xl">
...
export function TicketsView() {
  ...
  const activeCount = tickets.filter(t => 
    t.status && t.status !== 'resolved' && t.status !== 'closed'
  ).length
  ...
  return (
    ...
    >
      ...
      {creating && createPortal(
        <AnimatePresence>
          ...
          >
            ...
            >
              <h4 className="text-lg font-black mb-4">Create Support Ticket</h4>
              <CreateTicketForm
        ...
        {loading ? (
          ...
          tickets.map((ticket, idx) => (
            <motion.div
              ...
              transition={{ delay: idx * 0.1, duration: 0.2, ease: [0.16, 1, 0.3, 1] }}
              ...
              <div className="flex items-center gap-5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-[#9A9A9A] font-bold uppercase tracking-wide">
                    {ticket.room_number} • {new Date(ticket.created_at).toLocaleDateString()}
                  </p>
                  <p className="text-xs text-[#79837C] mt-1 line-clamp-1">{ticket.description}</p>
                </div>
              </div>
                ...
                <div className="text-right">
                  <div className="flex items-center gap-2 justify-end mb-1">
                    <span className={`w-2 h-2 rounded-full ${
                      ticket.status === 'in_progress' ? 'bg-blue-500 animate-pulse' 
                  ...
                  <p className="text-[10px] text-[#9A9A9A] font-bold">
                    Updated {ticket.updated_at ? new Date(ticket.updated_at).toLocaleDateString() : 'N/A'}
                  </p>
                </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-4">
        {[
          { label: 'Active', value: activeCount, icon: Clock, color: 'text-blue-500' },
          { label: 'Resolved', value: resolvedCount, icon: CheckCircle2, color: 'text-emerald-500' },
          { label: 'Waiting', value: waitingCount, icon: MessageSquare, color: 'text-orange-500' }
        ].map((stat, i) => (
